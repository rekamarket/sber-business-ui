...
@module("./SberButtonStyle.css.js") external classNameRoot: string = "className"
...
let {displayName} = module(SberButtonMeta)
...
let make = (
  ...
  <button
    className={Cn.make([
      ...
      LitonSberLayer.resolve(~size, ~variant),
    ])}
    ?style
    ?tabIndex
    ?onBlur
    ...
    <svg
      ...
      | Some(b) =>
        switch b {
        | true => classNameAppearOut
        | false => classNameAppearIn
        }
